...
They were run in OS X 10.10.2 with the following bash commands

export R_LIBS=../../../nleqslv.Rcheck
for k in *.R; do R CMD BATCH --no-timing $k ; done

from the nleqslv source directory.
...
for file in *.Rout ; do  gawk '/  Iter /, /^[$]x/  { print }' $file > $(basename $file).txt ; done
